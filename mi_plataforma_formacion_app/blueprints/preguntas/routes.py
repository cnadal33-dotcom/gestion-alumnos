...
def lista_respuestas(pregunta_id):
    ...
    # Mostrar sólo las respuestas pertenecientes al modelo (no las respuestas de alumnos)
    respuestas = Respuesta.query.filter_by(pregunta_id=pregunta_id, examen_alumno_id=None).all()
...
def nueva_respuesta(pregunta_id):
    form = RespuestaForm()
    if form.validate_on_submit():
        # Evitar considerar respuestas de alumno al comprobar duplicados o al forzar la no-correcta
        if form.es_correcta.data:
            Respuesta.query.filter_by(pregunta_id=pregunta_id, examen_alumno_id=None).update({"es_correcta": False})
        # Crear la nueva respuesta del modelo (examen_alumno_id debe ser NULL)
        r = Respuesta(texto=form.texto.data, es_correcta=form.es_correcta.data, pregunta_id=pregunta_id, examen_alumno_id=None)
        db.session.add(r)
        db.session.commit()
...
def editar_respuesta(id):
    ...
    if form.validate_on_submit():
        if form.es_correcta.data:
            # Forzar que sólo las respuestas de modelo cambien su flag de correcta
            Respuesta.query.filter_by(pregunta_id=respuesta.pregunta_id, examen_alumno_id=None).update({"es_correcta": False})
