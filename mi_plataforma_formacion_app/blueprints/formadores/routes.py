...
@formadores_bp.route('/info/<int:id>')
def info_formador(id):
    formador = Formador.query.get_or_404(id)
    tipos = [ { 'id': tc.id, 'nombre': tc.nombre } for tc in formador.tipos_curso ]
    firma_url = None
    if formador.firma:
        # return a static URL for the signature image when available
        try:
            firma_url = url_for('static', filename='firmas_formadores/' + formador.firma)
        except Exception:
            firma_url = formador.firma
