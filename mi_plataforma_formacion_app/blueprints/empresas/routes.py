...
@empresas_bp.route('/api/empresas/datalist')
def api_empresas_datalist():
    """Devuelve una lista plana de empresas (hasta 500) para poblar un <datalist> en el cliente."""
    q = request.args.get('q', '').strip()
    empresas_q = Empresa.query
    if q:
        empresas_q = empresas_q.filter(Empresa.nombre.ilike(f'%{q}%'))
    empresas = empresas_q.order_by(Empresa.nombre).limit(500).all()
    return jsonify([{'id': e.id, 'nombre': e.nombre} for e in empresas])