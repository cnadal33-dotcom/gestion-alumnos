...
def alumnos_formacion(formacion_id):
    ...
    formacion = Formacion.query.get_or_404(formacion_id)
    # Permitir la vista en GET para administradores; bloquear solo si estamos en modo 'student' público
    from flask import g
    if getattr(formacion, 'cerrada', False):
        # Si la petición es de tipo GET y no es un flujo público de estudiante, permitimos la vista en modo solo-lectura
        if request.method != 'GET' or getattr(g, 'student_mode', False):
            try:
                current_app.logger.info('Intento de acceso a alumnos_formacion en formacion CERRADA (bloqueado)', extra={'id': getattr(formacion, 'id', None), 'remote_addr': request.remote_addr})
            except Exception:
                pass
...
def agregar_alumno_existente(formacion_id):
    ...
    if getattr(formacion, 'cerrada', False):
        try:
            current_app.logger.info('Intento de agregar_alumno_existente en formacion CERRADA', extra={'id': getattr(formacion, 'id', None), 'remote_addr': request.remote_addr})
        except Exception:
            pass
        return render_template('formaciones/formacion_cerrada.html', formacion=formacion), 403
...
def agregar_alumno_nuevo(formacion_id):
    ...
    if getattr(formacion, 'cerrada', False):
        try:
            current_app.logger.info('Intento de agregar_alumno_nuevo en formacion CERRADA', extra={'id': getattr(formacion, 'id', None), 'remote_addr': request.remote_addr})
        except Exception:
            pass
        return render_template('formaciones/formacion_cerrada.html', formacion=formacion), 403
    form = AlumnoForm()
...
def realizar_examen(alumno_formacion_id):
    ...
    for pregunta in examen_modelo.preguntas:
        # Construir lista de opciones únicas por texto, preservando orden
        seen = set()
        opts = []
        for resp in pregunta.respuestas:
            if resp.examen_alumno_id is not None:
                continue
        ...
        for resp in pregunta.respuestas:
            if resp.examen_alumno_id is None and resp.es_correcta:
                respuesta_correcta = (resp.texto or '').strip()
                break

        preguntas_test.append({
            'id': pregunta.id,
            'pregunta': pregunta.texto,
            # Usar sólo las opciones únicas del modelo (examen_alumno_id es NULL)
            'opciones': opts,
            'respuesta_correcta': respuesta_correcta
        })

    examen_alumno = ExamenAlumno.query.filter_by(alumno_formacion_id=alumno_formacion_id).first()
    # notas: ahora almacenamos respuestas como filas en la tabla `respuesta` con examen_alumno_id
    respuestas_previas = {}
    ...
    if request.method == 'POST':
        ...
        for pregunta in preguntas_test:
            ...
            # Normalizar texto (quitar espacios y comparar en minúsculas para robustez)
            def norm(x):
                try:
            ...
            respuesta_norm = norm(respuesta_dada) if respuesta_dada is not None else None
            respuestas[str(pregunta['id'])] = respuesta_norm
            correcta_raw = pregunta.get('respuesta_correcta')
            correcta_norm = norm(correcta_raw) if correcta_raw is not None else None
            if respuesta_norm is not None and correcta_norm is not None and respuesta_norm == correcta_norm:
                correctas += 1
        nota = round((correctas / len(preguntas_test)) * 10, 2) if preguntas_test else 0
        # Guardar/actualizar ExamenAlumno y crear filas en `respuesta` para las respuestas del alumno
        # Importar solo Respuesta desde el paquete para evitar declarar ExamenAlumno
        # como variable local en esta función (ya está importado a nivel de módulo).
        from mi_plataforma_formacion_app.models import Respuesta as RespuestaModel
        ...
        # Eliminar respuestas previas del alumno para este examen (si existen)
        RespuestaModel.query.filter_by(examen_alumno_id=examen_alumno.id).delete()
        db.session.commit()

        # Insertar nuevas respuestas como filas (no tocar las respuestas del modelo)
        for pregunta in preguntas_test:
            respuesta_dada_id = respuestas.get(str(pregunta['id']))
            if not respuesta_dada_id:
                continue
            # Buscamos la opción original por texto o por id entre las opciones del modelo
            # Primero intentamos tratar respuesta_dada_id como id de opción
            opcion_text = None
            opcion_es_correcta = False
            try:
                # intentar int id -> buscar en las respuestas del modelo (si existen como IDs)
                opcion_id_int = int(respuesta_dada_id)
            except Exception:
                opcion_id_int = None
            # localizar la opción original
            opciones_originales = []
            # las preguntas_test solo contienen textos; para localizar correctamente usamos la entidad Pregunta
            from mi_plataforma_formacion_app.models import Pregunta
            pregunta_ent = Pregunta.query.get(pregunta['id'])
            if pregunta_ent:
                opciones_originales = [r for r in pregunta_ent.respuestas if r.examen_alumno_id is None]
            if opcion_id_int:
                opt = next((o for o in opciones_originales if o.id == opcion_id_int), None)
                if opt:
                    opcion_text = opt.texto
                    opcion_es_correcta = bool(opt.es_correcta)
            if opcion_text is None:
                # fallback: la respuesta_dada puede ser el texto de la opción
                # comparar normalizando texto (usa lower para robustez)
                def norm(x):
                    try:
                        return x.strip().lower()
                    except Exception:
                ...
                opt = next((o for o in opciones_originales if norm(o.texto) == norm(respuesta_dada_id)), None)
                if opt:
                    opcion_text = opt.texto
                    # No copiar la marca de correcta a la respuesta del alumno;
                    # las filas del alumno deben mantenerse separadas y
                    # no contener flags de corrección para evitar sobrescribir
                    # la plantilla del modelo.
                    opcion_es_correcta = bool(opt.es_correcta)
            # Si aún no hemos resuelto texto, usar el valor bruto
            if opcion_text is None:
                opcion_text = str(respuesta_dada_id)

            # Crear una NUEVA fila para la respuesta del alumno. No modificar
            # la fila del modelo original (examen_alumno_id is NULL).
            nueva_respuesta = RespuestaModel(
                texto=opcion_text,
                # Mantener es_correcta sólo en las filas del modelo; en las
                # respuestas del alumno la dejamos en False (o NULL según el
                # modelo) para evitar que se mezclen banderas de corrección.
                es_correcta=False,
        ...
        # ✅ Generar y guardar el diploma automáticamente si corresponde
        if nota >= 5:
            # La función crear_y_guardar_diploma necesita la plantilla como tercer argumento
            plantilla = tipo_curso.plantilla_diploma if tipo_curso else None
            try:
                crear_y_guardar_diploma(alumno_formacion.alumno, alumno_formacion.formacion, plantilla)
            except Exception:
                current_app.logger.exception("Error generando diploma automático en formaciones.realizar_examen for alumno_formacion_id=%s", alumno_formacion_id)
                flash("Se aprobó el examen pero falló la generación automática del diploma.", "warning")
...
def ver_examen_alumno(examen_alumno_id):
    ...
    # Cargar las respuestas guardadas del alumno desde la tabla `respuesta`
    ...
    for pregunta in preguntas_db:
        ...
        respuesta_alumno = respuestas_guardadas.get(pregunta_id)
        respuesta_dada = respuesta_alumno.texto if respuesta_alumno else 'No respondido'

        # Buscar la respuesta correcta entre las opciones originales del modelo
        ...
        es_correcta = False
        if respuesta_alumno and respuesta_correcta is not None:
            es_correcta = _norm(respuesta_dada) == _norm(respuesta_correcta)

        correccion_preguntas.append({
            'pregunta': safe_str(pregunta.texto),
            'respuesta_dada': safe_str(respuesta_dada),
            'respuesta_correcta': safe_str(respuesta_correcta),
...
def generar_acta(formacion_id, plantilla_id):
    ...
    # Intentar rellenar el PDF de forma robusta; si falla, mostrar flash y redirigir
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as temp_file:
            ruta_temporal = temp_file.name
        ok = rellenar_pdf(ruta_absoluta_plantilla, ruta_temporal, data)
        if not ok:
            # limpiar fichero temporal si fue creado vacio
            try:
                if os.path.exists(ruta_temporal):
                    os.remove(ruta_temporal)
            except Exception:
                pass
            flash('No se pudo generar el acta: plantilla inválida o error al rellenar el PDF.', 'danger')
            return redirect(request.referrer or url_for('formaciones_bp.ficha_formacion', formacion_id=formacion_id))
        return send_file(ruta_temporal, download_name=f"acta_{formacion_id}.pdf", as_attachment=True, mimetype='application/pdf')
    except Exception as e:
        current_app.logger.exception('Error generando acta para formacion %s con plantilla %s', formacion_id, plantilla_id)
        flash('Error interno generando acta. El administrador ha sido notificado.', 'danger')
        return redirect(request.referrer or url_for('formaciones_bp.ficha_formacion', formacion_id=formacion_id))
